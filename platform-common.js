...
var NativeScriptDocument = /** @class */ (function() {
  ...
  NativeScriptDocument.prototype.body = {
    isOverride: true
  };
  NativeScriptDocument.prototype.createElement = function(tag) {
    throw new Error(
      "NativeScriptDocument is not DOM Document. There is no createElement() method."
    );
  };
...
var NativeScriptPlatformRef = /** @class */ (function(_super) {
  ...
  NativeScriptPlatformRef.prototype.bootstrapModuleFactory = function(
    moduleFactory
  ) {
    var _this = this;
  ...
  NativeScriptPlatformRef.prototype.bootstrapModule = function(
    moduleType,
    compilerOptions
  ) {
    var _this = this;
  ...
  NativeScriptPlatformRef.prototype.bootstrapNativeScriptApp = function() {
    ...
    var rootContent;
    if (autoCreateFrame) {
      var _a = this.createFrameAndPage(false),
        page = _a.page,
        frame = _a.frame;
    ...
    var launchCallback = profiling_1.profile(
      ...
      function(args) {
        ...
        _this._bootstrapper().then(
          function(moduleRef) {
            bootstrapPromiseCompleted = true;
            trace_1.bootstrapLog(
              "Angular bootstrap bootstrap done. uptime: " +
                profiling_1.uptime()
            );
            if (!autoCreateFrame) {
        ...
        trace_1.bootstrapLog(
          "bootstrapAction called, draining micro tasks queue. Root: " +
            rootContent
        );
        global.Zone.drainMicroTaskQueue();
        trace_1.bootstrapLog(
          "bootstrapAction called, draining micro tasks queue finished! Root: " +
            rootContent
        );
  ...
  NativeScriptPlatformRef.prototype.livesync = function() {
    ...
    exports.onBeforeLivesync.next(
      lastBootstrappedModule ? lastBootstrappedModule.get() : null
    );
    var autoCreateFrame = !!this.appOptions.createFrameOnBootstrap;
    var tempAppHostView;
    var rootContent;
    if (autoCreateFrame) {
      var _a = this.createFrameAndPage(true),
        page = _a.page,
        frame = _a.frame;
      platform_providers_1.setRootPage(page);
      rootContent = frame;
    ...
    trace_1.bootstrapLog(
      "livesync bootstrapAction called, draining micro tasks queue. Root: " +
        rootContent
    );
    global.Zone.drainMicroTaskQueue();
    trace_1.bootstrapLog(
      "livesync bootstrapAction called, draining micro tasks queue finished! Root: " +
        rootContent
    );
    if (!bootstrapPromiseCompleted) {
  ...
  NativeScriptPlatformRef.prototype.createFrameAndPage = function(isLivesync) {
    ...
    var pageFactory = this.platform.injector.get(
      platform_providers_1.PAGE_FACTORY
    );
  ...
  __decorate(
    ...
    NativeScriptPlatformRef.prototype,
